...
function SkrrtSkrrt()
{
		...
		if(Entities.IsHero(query))
		{
			...
			if(pINs != undefined)
			{
				...
				if(pINs[query] != undefined)
				{
					...
					if(pIDs[Entities.GetPlayerOwnerID(query)].attack != undefined) dmgtype = pINs[query].attack;
					if(pIDs[Entities.GetPlayerOwnerID(query)].armor != undefined) armtype = pINs[query].armor;
				}
				else
				{
	...
	if(pINs != undefined)
	{
		...
		{
			$.Msg("why is it not a number "+pIDs[pOwner].strength);
			$.Msg("why is it not a number "+strBlast);
...
function Bah()
{
	...
	for (var playerID in pIDs) {
		$.Msg("please dont change or im get mad "+Players.GetPlayerHeroEntityIndex(parseInt(playerID)));
		if(pINs != undefined)//i have to do this check every time or the fuckin game will yell at me
		{
			if(pINs[Players.GetPlayerHeroEntityIndex(parseInt(playerID))] == undefined)
			{
				var hero = Players.GetPlayerSelectedHero(parseInt(playerID));
