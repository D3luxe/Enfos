...
function RadiantLoseLife(trigger)
	if trigger.activator:IsCreature() then 
		...
		GameRules:GetGameModeEntity():SetTopBarTeamValue(DOTA_TEAM_BADGUYS, _goodLives)
...
function DireLoseLife(trigger)
	if trigger.activator:IsCreature() then 
		...
		trigger.activator:ForceKill(true)
		GameRules:GetGameModeEntity():SetTopBarTeamValue(DOTA_TEAM_GOODGUYS, _badLives)
		if _badLives == 0 then
